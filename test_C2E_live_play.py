...
class TestOpenFlow(object):
    ...
    def test_public_open_flow_result(self, dev_model):
        ...
        gz_public.result_save_excel_column_list('C2E_result.xlsx', 'Sheet1', 1,
        ...
        with allure.step('step2: 点击前面拿到的设备屏幕,开始开流，例如，%s。step时间点：%s'
            ...
            '''
            1.  等待 loading。
            2.  进来就截图，10秒jie 20 zhang，长点10 秒还没有就判失败
            3.  看多线程咋搞。一边截图，一边找元素。
            '''
        ...
        with allure.step('step3: 进入开流页面，查看开流结果。step时间点：%s'
            ...
            # WebDriverWait(master, timeout=45, poll_frequency=1).until_not(
            ...
            for ii in range(1, 13):
                ...
                start_image_save_path = './report/C2E/%s_start_flow_%s.png' % (ii, start_flow)
            ...
            for jj in range(1, len(Screenshot_path_and_time_list) + 1):
                ...
                image_black_and_white_path_name = './report/C2E/%s_start_flow_%s.png' % (jj, start_flow)
                # 将 黑白截图 再节选出 需要的部分,保存到路径 image_partial_path_name 中
                image_partial_path_name = './report/C2E/%s-1_start_flow_%s.png' % (jj, start_flow)
                # 将 本次开流截图,转成黑白色,并且保存到 路径  image_black_and_white_path_name  中
                ...
                # start_extracted_text = gz_public.get_image_text_simple(image_black_and_white_path_name, 0, 650, 210,
                #                                                        1080, 260, image_partial_path_name)  # 手机 moto G

                # 将 黑白截图 再节选出 需要的部分,保存到路径 image_partial_path_name 中,并且识别 其中的文字 传给start_extracted_text
                start_extracted_text = gz_public.get_image_text_simple(image_black_and_white_path_name, 0, 600, 250,
                                                                       1080, 300, image_partial_path_name)  # 手机 pixel 7
                ...
                if start_extracted_text != "":
                    ...
                    # 将 开流成功时间 保存到 excel 中

                    # 将 点击屏幕时间 保存到Excel中.
                    colume_to_add_success = ['B']
                    gz_public.result_save_excel_column_list('C2E_result.xlsx', 'Sheet1', click_time,
                                                            colume_to_add_success)

                    # 将 开流成功的最后时间,保存到Excel中.
                    colume_to_add_success = ['C']
                    gz_public.result_save_excel_column_list('C2E_result.xlsx', 'Sheet1', success_live_play_time,
                                                            colume_to_add_success)
                    ...
                    if jj == len(Screenshot_path_and_time_list):
                        ...
                        colume_to_add_success = ['C']
                        gz_public.result_save_excel_column_list('C2E_result.xlsx', 'Sheet1', '开流失败',
                                                                colume_to_add_success)
                        ...
                        colume_to_add_success = ['P']
                        gz_public.result_save_excel_column_list('C2E_result.xlsx', 'Sheet1', 0,
                                                                colume_to_add_success)
        ...
        #
        #     # 日志中获取开流状态 和 开流状态的时间.
        #     play_state, play_state_time = gz_public.select_log_keyword_state_advanced(
        #         '获取 设备开流状态 ', 'LivePlayer', 'state',
        #         'state:Playing', -1, 'ZY22CXDW3K')
        #     print('开流状态: %s , 开流成功时间: %s ' % (play_state, play_state_time))
        #
        #     # 计算  从点击屏幕返回 到 设备休眠成功 的时间差值,将时间字符串 转化成 datetime 格式用于相减计算
        #     time_difference_success = gz_public.time_difference_medium(play_state_time, click_time)
        #     print('开流用时: %s ' % time_difference_success)
        #
        #     # 判断 日志中开流状态为 playing ,标志开流成功)
        ...
        #         # 将 开流成功的最后时间,保存到Excel中.
        #         colume_to_add_success = ['C']
        #         gz_public.result_save_excel_column_list('C2E_result.xlsx', 'Sheet1', play_state_time,
        #                                                 colume_to_add_success)
        #
        #         # 将 点击屏幕开流的次数 保存到Excel中.
        #         colume_to_add_success = ['O']
        ...
        #
        #         # 截一张图
        #         start_flow = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    ...
    def test_C5L_live_play(self):
        self.test_public_open_flow_result('C5L')

    @allure.title('C2E 开流')
    @allure.story('循环测试 摄像机 正常开流')
    def test_C2E_live_play(self):
        self.test_public_open_flow_result('C2E')
...
if __name__ == '__main__':
    ...
    # C2E 多次开流
    pytest.main(["-q", "-s", "-ra", "--count=%d" % 200, "test_C2E_live_play.py::TestOpenFlow::test_C2E_live_play",
