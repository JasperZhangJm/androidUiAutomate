...
devices = ['Pixel 7', 'Pixel 4', 'Samsung S8', 'moto g', 'Pixel 7', 'SamsungSCV36', 'SamsungA51', 'moto_z4']
...
class TestOpenFlow(object):
    ...
    def test_public_disconnect_flow_after_30_minutes(self, dev_model):
        ...
        with allure.step('step2: 点击前面拿到的设备屏幕,开始开流，例如，%s。step时间点：%s'
                         % (dev_name, time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))):
            master.find_element(AppiumBy.ID, 'com.huiyun.care.viewerpro:id/wifi_force_du_signal').click()
        ...
        with allure.step('step3: 开流开始后，开流时长 30分钟, 每 10秒 检查一次开流状态'):
            ...
            # 这些状态消失后，才成功开流
            while gz_public.isElementPresent(driver=master, by="id",
                                             value="com.huiyun.care.viewerpro:id/loading_progressbar") is True:
                time.sleep(1)
                break

            # 成功开流后记录开始的时间戳
            start_connect_flow_ts = datetime.now().replace(microsecond=0)
            print("开始开流时间是：", start_connect_flow_ts)

            # 开流开始后 截一张图
            start_flow = time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime())
            image_save_path = './report/Panda/start_flow_%s.png' % start_flow
            master.save_screenshot(image_save_path)
            time.sleep(3)

            # 将截图添加到报告中
            allure.attach.file("./report/Panda/start_flow_%s.png" % start_flow, name="start_flow_%s" % start_flow,
            ...
            for ii in range(1, 181):
                try:
                    ...
                    # 开流开始后 截一张图
                    start_flow = time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime())
                    start_image_save_path = './report/Panda/%s_start_flow_%s.png' % (ii, start_flow)
                    ...
                    # 将截图添加到报告中
                    allure.attach.file("./report/Panda/%s_start_flow_%s.png" % (ii, start_flow),
                    ...
                    image_black_and_white_path_name = './report/Panda/%s_start_flow_%s.png' % (ii, start_flow)
                    # 将 黑白截图 再节选出 需要的部分,保存到路径 image_partial_path_name 中
                    image_partial_path_name = './report/Panda/%s-1_start_flow_%s.png' % (ii, start_flow)
                    # 将 本次开流截图,转成黑白色,并且保存到 路径  image_black_and_white_path_name  中
                    ...
                    start_extracted_text = gz_public.get_image_text_simple(image_black_and_white_path_name, 0, 0, 280,
                                                                           500, 350, image_partial_path_name)

                    print(start_extracted_text)  # 例: dan.08.2024 20:18:27
                    # 获取 提取到的字符串中的  时间.
                    start_extracted_text_time = start_extracted_text.split(' ')[2]  # 例: 20:18:27
                    ...
                    next_image_save_path = './report/Panda/%s_start_flow_%s.png' % (ii, next_flow)
                    master.save_screenshot(next_image_save_path)

                    # 将截图添加到报告中
                    allure.attach.file("./report/Panda/%s_start_flow_%s.png" % (ii, next_flow),
                                       name="%s_start_flow_%s" % (ii, next_flow),
                                       attachment_type=allure.attachment_type.JPG)
                    master.implicitly_wait(10)

                    # 提取下一次 开流页面,左上角的时间戳.
                    picture_contrast_path_name = './report/Panda/%s_start_flow_%s.png' % (ii, next_flow)
                    ...
                    gz_public.result_save_excel_full_list('./Panda_result.xlsx', 'Sheet1', content_full_list_to_add,
                    ...
                    if ii == 1:
                        ...
                        gz_public.result_save_excel_column_list('Panda_result.xlsx', 'Sheet1', 1,
                        ...
                        gz_public.result_save_excel_column_list('./Panda_result.xlsx', 'Sheet1', 'pass',
                                                                column_full_list_to_add)
                        ...
                        if different_connect_flow_ts <= different_extracted_text_datetime + datetime_self.timedelta(
                            ...
                            # 将 执行失败 次数,统计在Excel中.
                            column_full_list_to_add = ['M', 'N']
                            gz_public.result_save_excel_column_list('./Panda_result.xlsx', 'Sheet1', 0,
                                                                    column_full_list_to_add)
                            master.implicitly_wait(10)

                            # 将 执行失败 结果,统计在Excel中.
                            column_full_list_to_add = ['E']
                            gz_public.result_save_excel_column_list('./Panda_result.xlsx', 'Sheet1', 'fail',
                                                                    column_full_list_to_add)
        ...
        #
        #         # 提取开流页面,左上角的时间戳.
        #         start_extracted_text = gz_public.get_image_text_simple(start_image_save_path, 2)
        ...
        #
        #         # 提取开流页面,左上角的时间戳.
        #         next_extracted_text = gz_public.get_image_text_simple(next_image_save_path, 2)
        #         print(next_extracted_text)
        #         connect_flow_ts = datetime.now()
        #         print("完成提取图片时间戳时间是：", connect_flow_ts)
        #         master.implicitly_wait(10)
        ...
        #         time.sleep(7)
        #
        #         if start_extracted_text == next_extracted_text:
        #             # 执行次数
        #             colume_to_add_success = ['K']
        #             gz_public.result_save_excel_column_list('Panda_result.xlsx', 'Sheet1', 1,
        #                                                     colume_to_add_success)
        #             master.implicitly_wait(10)
        #
        #             # 将 执行失败 次数,统计在Excel中.
        #             column_full_list_to_add = ['L', 'M']
        #             gz_public.result_save_excel_column_list('./Panda_result.xlsx', 'Sheet1', 0,
        #                                                     column_full_list_to_add)
        #             master.implicitly_wait(10)
        #             # 将 执行失败 结果,统计在Excel中.
        #             column_full_list_to_add = ['D']
        #             gz_public.result_save_excel_column_list('./Panda_result.xlsx', 'Sheet1', 'fail',
        #                                                     column_full_list_to_add)
        #             master.implicitly_wait(10)
        #
        #             assert '上次开流截图时间戳 " == "本次开流截图时间戳 ', "上次开流时间戳 %s 与本次开流时间戳 %s 一致,表示出现了卡顿" % (
        #                 start_extracted_text, next_extracted_text)
        #             master.implicitly_wait(10)
        #
        #         else:
        #             # 执行次数
        ...
        #             master.implicitly_wait(10)
        #
        #             assert '两次开流截图时间戳 " == "两次开流截图时间戳 ', "上次开流时间戳 %s 与本次开流时间戳 %s 不一致,表示没有出现了卡顿" % (
        #                 start_extracted_text, next_extracted_text)
        #             master.implicitly_wait(10)
        #
        #         ii = ii + 1
        ...
        with allure.step('step5：点击页面左上角的 返回，结束开流。step时间点：%s 。开流到断流的时间差: %s '
            ...
            # 点击左上角的 返回按钮
            master.find_element(AppiumBy.ID, 'com.huiyun.care.viewerpro:id/double_screen_back_icon_iv').click()
            master.implicitly_wait(10)
        ...
        with allure.step('step6: 冷启app。step时间点：%s'
            ...
            # master.close_app()
            master.implicitly_wait(10)
            master.terminate_app(app_id=android_package_name, timeout=2000)  # 查下 pid 是否存在.
            master.implicitly_wait(10)

    @allure.title('panda 设备开流30分钟后,自动断流')
    @allure.story('用户使用设备开流30分钟后,自动断流')
    def test_panda_live_play(self):
        self.test_public_disconnect_flow_after_30_minutes('panda')

    @allure.title('ring 设备开流30分钟后,自动断流')
    @allure.story('用户使用设备开流30分钟后,自动断流')
    def test_ring_live_play(self):
        self.test_public_disconnect_flow_after_30_minutes('ring')
...
if __name__ == '__main__':
    ...
    # ring 多次开流
    pytest.main(
        ["-q", "-s", "-ra", "--count=%d" % 10, "test_Huiyun_live_play_stability.py::TestOpenFlow::test_panda_live_play",
         "--alluredir=./report/Panda"])
